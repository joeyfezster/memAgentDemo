...
async def test_agent_uses_logged_in_user_name() -> None:
    ...
    for email, password, user_message, expected_name, expected_word in test_cases:
        with patch(
            "app.services.agent_service.AgentService.generate_response_with_tools",
            new_callable=AsyncMock,
        ) as mock_generate:
            mock_generate.return_value = (
                "Hi Joe, pleasure to meet you. banana.",
...
async def test_multi_user_isolation() -> None:
    with patch(
        ...
        mock_generate.side_effect = [
            ("Hi Joe, nice to meet you. banana.", {"stop_reason": "end_turn"}),
            ("Hello! I don't know your name.", {"stop_reason": "end_turn"}),
        ]
        async with AsyncClient(app=app, base_url="http://test") as client:
            ...
            daniel_conv = await client.post(
                "/chat/conversations", headers=daniel_headers
            )
            ...
            assert (
                sarah_response["user_message"]["conversation_id"]
            ...
            assert (
                daniel_response["user_message"]["conversation_id"]
            ...
            sarah_assistant_content = sarah_response["assistant_message"][
                "content"
            ].lower()
            assert (
