---
title: Self-hosting Letta | letta-sdk
description: Learn how to self-host Letta servers with your own infrastructure and configuration.
---

Note

The recommended way to use Letta locally is with Docker. To install Docker, see [Docker’s installation guide](https://docs.docker.com/get-docker/). For issues with installing Docker, see [Docker’s troubleshooting guide](https://docs.docker.com/desktop/troubleshoot-and-support/troubleshoot/).
...
To enable tool sandboxing, set the `E2B_API_KEY` and `E2B_SANDBOX_TEMPLATE_ID` environment variables (via [E2B](https://e2b.dev/)) when you use `docker run`. When sandboxing is enabled, all custom tools (created by users from source code) will be executed in a sandboxed environment.
